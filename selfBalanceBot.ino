...
// -- -- -- -- -- -- -- -- -- -- -- -- -- --
#define DIR_A 3                // Direction Pin, Motor A
#define DIR_B 5                // Direction Pin, Motor B
#define PWM_A 9                 // PWM, Motor A (Left Motor)
#define PWM_B 10                // PWM, Motor B (Right Motor)
#define BRK_A 4                 // Brake, Motor A
#define BRK_B 6                 // Brake, Motor B

#define BTN_1 11                 // ON/OFF Button
#define BTN_2 12                 // Calibration(Set Centre of Gravity) Button

#define LED_1 7                // Calibration LED
#define LED_2 8                // ON/OFF LED
...
// For PID Controller
// -- -- -- -- -- -- -- -- -- -- -- -- -- --
float Kp = 50;                   // (P)roportional Tuning Parameter
...
float targetAngle = 1;        // Can be adjusted according to centre of gravity

// You can Turn off YAW control, by setting
// the Tp and Td constants below to 0.
float Tp = 0;             // Yaw Proportional Tuning Parameter
float Td = 0;         // Yaw Derivative Tuning Parameter
...
void setup() {
  ...
  pinMode(PWM_A, OUTPUT);     // Right Motor PWM
  pinMode(PWM_B, OUTPUT);     // Right Motor PWM
...
int PID(float pitch) {
  ...
  if(PIDValue > 0) PIDValue = PIDValue + 30;
  if(PIDValue < 0) PIDValue = PIDValue - 30;
...
float pitch() {
  return (ypr[1] * 18 / M_PI);
}

float yaw() {
  return (ypr[0] * 18 / M_PI);
}
...
void loop() {
  ...
  if (modeSelect == 1) {
    ...
    if (!initialised) {
      ...
      if ((pitch() < targetAngle + 0.5) && (pitch() > targetAngle - 0.5) && (abs(angRate()) < 0.3)) {
